...
gnome-terminal -t "arm calib" -x bash -c "cd ~/RJ1400/robot/ && source devel/setup.bash && roslaunch ur_calibration calibration_correction.launch robot_ip:=192.168.1.52;exec bash"
sleep 3
...
gnome-terminal -t "bring up" -x bash -c "cd ~/RJ1400/robot/ && source devel/setup.bash && roslaunch ur_modern_driver ur10_bringup.launch limited:=true robot_ip:=192.168.1.52;exec bash"
sleep 3
...
gnome-terminal -t "easy_hand_eye" -x bash -c "cd ~/RJ1400/calibration/ && source devel/setup.bash && roslaunch easy_handeye easy_hand_server_only.launch namespace_prefix:=/ur10_rightarm;exec bash"